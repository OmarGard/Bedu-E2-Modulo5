...
def clasification_comparative(y_esperada,y_predicha,y_esperada2,y_predicha2):
    ...
    if accuracy[0] > accuracy[1]:
        print(f'Clasficador 1 es mejor que clasificador 2 en cuanto a precisión: {accuracy[0]} > {accuracy[1]}')
    else:
        print(f'Clasficador 2 es mejor que clasificador 1 en cuanto a precisión: {accuracy[1]} > {accuracy[0]}')

    if sensitivity[0] > sensitivity[1]:
        print(f'Clasficador 1 es mejor que clasificador 2 en cuanto a sensibilidad: {sensitivity[0]} > {sensitivity[1]}')
    else:
        print(f'Clasficador 2 es mejor que clasificador 1 en cuanto a sensibilidad: {sensitivity[1]} > {sensitivity[0]}')

    if specificity[0] > specificity[1]:
        print(f'Clasficador 1 es mejor que clasificador 2 en cuanto a specificidad: {specificity[0]} > {specificity[1]}')
    else:
        print(f'Clasficador 2 es mejor que clasificador 1 en cuanto a specificidad: {specificity[1]} > {specificity[0]}')
